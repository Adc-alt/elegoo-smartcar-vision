...
# `python src/main.py` no tiene paquete padre; hace falta `src.*` desde la raíz del repo.
...
from src.vision import detect_green_ball

IP = "192.168.4.1"
# ASCII title avoids rare Win32/OpenCV issues with non-ASCII window names
WINDOW = "SmartCar vision (BGR + mask)"
...
def _placeholder_frame(msg: str) -> np.ndarray:
    """Visible feedback while waiting for the first frame (must call waitKey)."""
    img = np.zeros((480, 640, 3), dtype=np.uint8)
    cv2.putText(
        img, msg, (30, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (200, 200, 200), 2
    )
    cv2.putText(
        img, f"http://{IP}/stream", (30, 240), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (120, 180, 120), 1
    )
    cv2.putText(
        img, "Close other viewers (browser/app) if only one client allowed.", (30, 280),
        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (100, 100, 200), 1,
    )
    cv2.putText(
        img, "Press q to quit", (30, 320), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (180, 180, 180), 1,
    )
    return img


def main():
    print(f"Connecting to camera at http://{IP}/stream ...", flush=True)
    print("If the window stays black: install opencv-python (not opencv-python-headless).", flush=True)
    camera = Camera(ip=IP)
    ...
    try:
        while True:
            ...
            if img_bgr is None:
                no_frame += 1
                if no_frame == 1 or no_frame % 60 == 0:
                    print(
                        f"[{no_frame}] No frame yet (WiFi to car? browser closed? /stream reachable?)",
                        flush=True,
                    )
                try:
                    cv2.imshow(WINDOW, _placeholder_frame("Waiting for camera frames..."))
                except cv2.error as e:
                    print("cv2.imshow failed (often opencv-python-headless):", e, flush=True)
                    print("Fix: pip uninstall opencv-python-headless -y && pip install opencv-python", flush=True)
                    return
                ...
                continue
            if no_frame > 0:
                print(f"First frame after {time.monotonic() - t0:.1f}s", flush=True)
            no_frame = 0
